...
CONFIG = {
    ...
    "test_path":  "data/processed/csi300_test_with_regime.parquet",
    "seq_len":    20,
    "feature_cols": [
    ...
    # MCD模型路径
    "mcd_checkpoint": "checkpoints/best_model.pt",
    "mcd_config": {
        "seq_length":         20,
        "feature_size":       10,
        "n_regimes":          3,
    ...
    "output_dir": "downstream/results",
    "n_generate": 100000,    # 每个regime生成多少样本
}


# ── 生成增强数据 ──────────────────────────────────────────────
def generate_augmented_data(cfg: dict) -> np.ndarray:
    """加载MCD模型，为每种regime生成合成数据，返回(N, T, F)"""
    ...
    ref_x = torch.tensor(
        df[cfg["feature_cols"]].dropna().values[:1000].astype(np.float32)
    ).unsqueeze(0).to(device)   # (1, 1000, F)，随便取一段做参考
    ...
    for regime_id in range(cfg["mcd_config"]["n_regimes"]):
        ...
        for batch_idx, start in enumerate(range(0, n_per_regime, batch_size)):
            ...
            # 用训练集的一个小batch做RevIN参考
            ref_batch = torch.tensor(
                sample_ref(curr_bs), dtype=torch.float32
            ).to(device)
        ...
        all_generated.append(np.concatenate(regime_samples, axis=0))

    result = np.concatenate(all_generated, axis=0)  # (N, T, F)
    print(f"  生成完成，总样本数: {result.shape}")
    return result
...
def main():
    ...
    # # ── 生成所有增强数据（各方法）
    # augmented_mcd    = generate_augmented_data(cfg)          # MCD（你的方法）
    # augmented_uncond = generate_unconditional_diffusion(cfg, n_samples=cfg["n_generate"])  # 消融
    # augmented_boot   = generate_bootstrap(cfg, n_samples=cfg["n_generate"])                # Bootstrap
